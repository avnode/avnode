...
const fetchUser = (id, cb) => {
  ...
  .populate([{
    ...
    populate: [{
      ...
      path: 'organisers', // virtual relation
    }, {
      path: 'organising_crews', // virtual relation
    }]
  },{
    path: 'performances',
    model: 'Performance',
...
router.put('/event/:id/organiser/:organiserId', (req, res) => {
  User
  .findById(req.params.organiserId)
  .exec((err, organiser) => {
    organiser.events.push(req.params.id);
    organiser.save((_err) => {
      // FIXME: Handle error…
      fetchUser(req.user.id, (err, user) => {
        res.json(user);
...
router.delete('/event/:id/organiser/:organiserId', (req, res) => {
  User
  .findById(req.params.organiserId)
  .exec((err, organiser) => {
    organiser.events.remove(req.params.id);
    organiser.save((_err) => {
      // FIXME: Handle error…
      fetchUser(req.user.id, (err, user) => {
        res.json(user);
...
router.put('/crew/:id', (req, res) => {
  ...
  Crew
  .findByIdAndUpdate(req.params.id, props)
  .exec((err) => {
    console.log(err);
    fetchUser(req.user.id, (err, user) => {
      res.json(user);
...
router.post('/performance/:id/video', (req, res) => {
  assetUtil.createVideoAsset(req.body.video, (err, assetId) => {
    Performance.findByIdAndUpdate(req.params.id, {
      video: assetId
    })
    .exec((err) => {
      console.log(err);
...
router.put('/user/:id', (req, res) => {
  ...
  User
  .findByIdAndUpdate(req.user.id, props)
  .exec((err) => {
    console.log(err);
    fetchUser(req.user.id, (err, user) => {
      res.json(user);
